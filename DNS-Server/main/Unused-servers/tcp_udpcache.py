        ...
        try:
            ...
            if entry_cache == 0:
                ...
                with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as upstream_socket:
                    ...
                    cache.insert(qname, upstream_dns_response)
                ...
                response = DNSRecord(
                    ...
                    q=request.q,
                    a=RR(qname, QTYPE.A, rdata=A(entry_cache[qname]["ip"]), ttl=entry_cache[qname]["ttl"])
                )

                client_socket.sendto(response.pack(), self.client_address)
                print("Served from cache:", qname, "->", cached_ip)
